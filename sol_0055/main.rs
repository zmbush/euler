...
use libeuler::traits::{DigitsHelper, PalindromeHelper};
...
/// less than fifty iterations, or, (ii) no one, with all the computing power that exists, has
/// managed so far to map it to a palindrome. In fact, 10677 is the first number to be shown to
/// require over fifty iterations before producing a palindrome: 4668731596684224866951378664 (53
...
fn main() {
    solutions! {
        sol naive {
            (0i64..10_000).filter(|n| is_lychrel(n)).count()
        }
    }
}

fn is_lychrel<T>(i: &T) -> bool
    where T: CheckedAdd + PalindromeHelper + DigitsHelper + ToBigInt {

    let mut m = i.clone();
    for _ in 0..50 {
        match m.checked_add(&m.reverse()) {
            Some(v) => m = v,
            None => return is_lychrel(&i.to_bigint().unwrap())
        }
        if m.is_palindrome() {
            return false;
        }
